...
try {
    if (isset($_REQUEST["invoiceid"]) && !empty($_REQUEST["invoiceid"])) {
        ...
        $savedPaymentId = HitpayHelper::getPaymentResponseSingle($invoiceid, 'payment_id');
        $reference = HitpayHelper::getSanitizedText($_REQUEST['reference']);
        if ($savedPaymentId != $reference) {
            throw new \Exception('Transaction references check failed. ');
        }

        HitpayHelper::redirect($redirectUrl);
    } else {
